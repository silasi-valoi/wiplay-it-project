...
from auth_app.utils import (_get_pin,
                                send_pin,
...
class User(AbstractBaseUser, PermissionsMixin, GuardianUserMixin):
    ...
    def follow(self, user, perms:dict):
        
        profile = user.profile        
        profile.followings = profile.followings + 1
        self.profile.followers = self.profile.followers + 1
        print(perms)

        assign_perm(perms['followers_perms'], user, self)
        assign_perm(perms['followings_perms'], self, user)       

        profile.save() 
        self.profile.save()

    def unfollow(self, user, perms:dict):
        
        profile = user.profile        
        profile.followings = profile.followings + 1
        self.profile.followers = self.profile.followers - 1

        remove_perm(perms['followers_perms'], user, self)
        remove_perm(perms['followings_perms'], self, user)
